...
pub fn __rust_to_wgsl_paste2(stream: TokenStream) -> TokenStream {
    ...
    let Some(TokenTree::Ident(definition)) = iter.next() else {
        abort!(
            Span::call_site(),
            "Expected `__rust_to_wgsl_paste!($definition {to_be_pasted} $([$($defined)*])? $($tt)*)`!"
        )
    };
    let Some(TokenTree::Group(pasted)) = iter.next() else {
        abort!(
    ...
    match iter.next() {
        ...
        Some(TokenTree::Group(g)) if g.delimiter() == Delimiter::Bracket => {
            ...
            let names: Vec<_> = g
                ...
                .collect();
            if found {
                quote!(::wgsl_ln::rust_to_wgsl!([#(#names)*] #tokens))
            } else {
                quote!(::wgsl_ln::rust_to_wgsl!([#(#names)* #definition] #pasted #tokens))
            }
        }
        // If no values defined, paste and define this item.
        other => {
            let tokens: TokenStream = iter.collect();
            quote! {
                ::wgsl_ln::rust_to_wgsl!([#definition] #pasted #other #tokens)
            }
        }
    }
}
